...
echo  "$($PSQL "SELECT t1.name FROM games left join teams as t1 on games.winner_id = t1.team_id or games.opponent_id = t1.team_id where year = 2014 and round = 'Eighth-Final' order by name")"  

echo -e "\nList of unique winning team names in the whole data set:"
echo  "$($PSQL "SELECT t1.name FROM games left join teams as t1 on games.winner_id = t1.team_id group by t1.name order by name")"
