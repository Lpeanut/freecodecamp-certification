  ...
  then
    ...
    then
      ...
      then
        ...
        RENTAL_ID=$($PSQL "select rental_id from customers inner join rentals using(customer_id) where date_returned is null and phone = '$PHONE_NUMBER' and bike_id = $BIKE_ID_TO_RETURN")
        # if input not rented
        if [[ -z $RENTAL_ID ]]
        then
          # send to main menu
